...
------TẠO PROC DELETE
ALTER PROC SP_NHANVIEN_DELETE
	@MANV INT
AS
BEGIN
	
	UPDATE dbo.PHIEUNHAP SET MA_NV = NULL WHERE MA_NV = @MANV
	UPDATE dbo.PHIEUXUAT SET MA_NV = NULL WHERE MA_NV = @MANV
	DELETE dbo.NHANVIEN WHERE MA_NV = @MANV
END
GO

DROP TRIGGER XOANV ON dbo.NHANVIEN INSTEAD OF DELETE
AS
	DECLARE @MANV INT
	BEGIN 
		SELECT @MANV = MA_NV FROM Deleted

		UPDATE dbo.PHIEUNHAP SET MA_NV = 0
		WHERE MA_NV = @MANV

		UPDATE dbo.PHIEUXUAT SET MA_NV = 0
		WHERE MA_NV = @MANV
	END
GO
...
AS
	BEGIN
	SELECT dbo.CT_PHIEUXUAT.MA_CTPX, CT_PHIEUXUAT.MA_PX, dbo.PHIEUXUAT.MA_NV, dbo.NHANVIEN.TEN_NV, NGAYXUAT, dbo.CT_PHIEUXUAT.MA_SP, TEN_SP, dbo.CT_PHIEUXUAT.SOLUONG, DONGIA, THANHTIEN = DONGIA*CT_PHIEUXUAT.SOLUONG
	FROM dbo.PHIEUXUAT, dbo.CT_PHIEUXUAT, dbo.SANPHAM, dbo.NHANVIEN
	WHERE dbo.PHIEUXUAT.MA_PX = dbo.CT_PHIEUXUAT.MA_PX AND dbo.CT_PHIEUXUAT.MA_SP = dbo.SANPHAM.MA_SP AND dbo.CT_PHIEUXUAT.MA_PX = @Maphieuxuat AND dbo.PHIEUXUAT.MA_NV = dbo.NHANVIEN.MA_NV
	END
GO
...
AS
	BEGIN
	SELECT dbo.CT_PHIEUNHAP.MA_PN, dbo.PHIEUNHAP.MA_NV, dbo.NHANVIEN.TEN_NV, NGAYNHAP, dbo.NHACUNGCAP.TEN_NCC, dbo.CT_PHIEUNHAP.MA_SP, TEN_SP, dbo.CT_PHIEUNHAP.SOLUONG, DONGIA, THANHTIEN= DONGIA*dbo.CT_PHIEUNHAP.SOLUONG
	FROM dbo.PHIEUNHAP, dbo.CT_PHIEUNHAP, dbo.NHACUNGCAP, dbo.NHANVIEN, dbo.SANPHAM
	 WHERE dbo.PHIEUNHAP.MA_PN = dbo.CT_PHIEUNHAP.MA_PN AND dbo.CT_PHIEUNHAP.MA_SP = dbo.SANPHAM.MA_SP 
				AND dbo.PHIEUNHAP.MA_NV = dbo.NHANVIEN. MA_NV AND dbo.PHIEUNHAP.MA_NCC = dbo.NHACUNGCAP.MA_NCC 
				AND dbo.PHIEUNHAP.MA_PN = @Maphieunhap
	END
 GO
...
AS
BEGIN
	SELECT *, dbo.NHACUNGCAP.TEN_NCC
	FROM dbo.SANPHAM, dbo.NHACUNGCAP
	WHERE dbo.SANPHAM.MA_NCC = dbo.NHACUNGCAP.MA_NCC
END
GO

CREATE PROC SP_TTSANPHAM_GETALL
AS
	BEGIN
	SELECT MA_SP, TEN_SP, THONGSO_KT, GIA, NHACUNGCAP.MA_NCC, TEN_NCC
	FROM dbo.SANPHAM, dbo.NHACUNGCAP
	WHERE dbo.SANPHAM.MA_NCC = dbo.NHACUNGCAP.MA_NCC
END
GO
...
CREATE PROC SP_TTSANPHAM_INSERT
	@TENSP NVARCHAR(100),
	@MANCC CHAR(10),
	@THONGSOKT NVARCHAR(100)
AS
	...
	VALUES  ( @TENSP , -- TEN_SP - nvarchar(100)
	          @MANCC , -- MA_NCC - int
	          @THONGSOKT  -- THONGSO_KT - nvarchar(100)          
	        )
END
GO
...
CREATE PROC SP_TTSANPHAM_UPDATE
	@MASP INT,
	@TENSP NVARCHAR(100),
	@MANCC INT,
	@THONGSOKT NVARCHAR(100)
...
BEGIN
	UPDATE dbo.SANPHAM
	SET TEN_SP=@TENSP, MA_NCC = @MANCC, THONGSO_KT = @THONGSOKT
	WHERE MA_SP = @MASP
END
GO
...
AS
BEGIN
	SELECT MA_SP, TEN_SP, dbo.SANPHAM.MA_NCC,THONGSO_KT,GIA,SOLUONG, dbo.NHACUNGCAP.TEN_NCC
	FROM dbo.SANPHAM, dbo.NHACUNGCAP
...
CREATE PROC SP_TTSANPHAM_SEARCH
	@SEARCHVALUE NVARCHAR(100)
AS
BEGIN
	SELECT MA_SP, TEN_SP, dbo.SANPHAM.MA_NCC, THONGSO_KT, dbo.NHACUNGCAP.TEN_NCC, GIA
	FROM dbo.SANPHAM, dbo.NHACUNGCAP
	WHERE (dbo.SANPHAM.MA_SP LIKE N'%' + @SEARCHVALUE +'%')
		OR (dbo.SANPHAM.TEN_SP LIKE N'%' + @SEARCHVALUE + '%')
		OR (dbo.SANPHAM.MA_NCC LIKE N'%' + @SEARCHVALUE + '%')
...
CREATE PROC THONGKE_GETALL
AS
	BEGIN
		SELECT dbo.SANPHAM.MA_SP, TEN_SP, SUM(dbo.CT_PHIEUXUAT.SOLUONG) ,NGAYXUAT, DONGIA, THANHTIEN=(SUM(dbo.CT_PHIEUXUAT.SOLUONG)*DONGIA)
		FROM dbo.SANPHAM, dbo.PHIEUXUAT, dbo.CT_PHIEUXUAT
		WHERE dbo.SANPHAM.MA_SP = dbo.CT_PHIEUXUAT.MA_SP AND dbo.PHIEUXUAT.MA_PX = dbo.CT_PHIEUXUAT.MA_PX
		GROUP BY dbo.SANPHAM.MA_SP, TEN_SP, DONGIA, NGAYXUAT
	END
GO
...
SELECT * FROM dbo.V_THANHTIEN
GO

CREATE VIEW v_thongke 
AS 
...
CREATE PROC THONGKE_GETALL(@TUNGAY DATE, @DENNGAY DATE)
AS
	BEGIN
